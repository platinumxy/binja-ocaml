...
class _RenameRegsTask(BackgroundTaskThread):
    ...
    def __update_func(self, func):
        ...
        yng_cnt = yng_existing = sum(var.name.startswith(
            "caml_young_ptr") for var in func.vars)
        err_cnt = err_existing = sum(var.name.startswith(
            "caml_exeption_ptr") for var in func.vars)
